...
CNode* CNode::GetBrother(int &flag) {
    CNode* pFather = GetFather(); // 获取父亲节点指针
    if (pFather == NULL) {
        return NULL;
    }
    CNode* pBrother = NULL;
    for (int i = 1; i <= pFather -> GetCount() + 1; ++i) {
        //GetCount()表示获取数据或关键字数，要比指针数小1。
...
// 在中间结点中删除键，以及该键后的指针
bool CInternalNode::Delete(KEY_TYPE value) {
    int i, j, k;
    for (i = 0; (value >= m_Keys[i]) && (i < m_Count); ++i) {}
...
KEY_TYPE CInternalNode::Split(CInternalNode* pNode, KEY_TYPE key) { //key是新插入的值，pNode是分裂结点
    ...
    // 判断是提取第V还是V+1个键
    int position = 0;
    if (key < this ->GetElement(ORDER_V)) position = ORDER_V;
    else position = ORDER_V + 1;
...
bool CInternalNode::Combine(CNode* pNode) {
    // 参数检查
    if (this->GetCount() + pNode->GetCount() + 1> MAXNUM_DATA)    // 预留一个新键的位置，给pNode提取一个节点作为位置
    {
        return false;
    }
...
bool CInternalNode::MoveOneElement(CNode *pNode) {
    ...
    if (pNode ->GetElement(1) < this -> GetElement(1)) {
        ...
        // 提取第一个孩子的第一个键作为本节点的开始，这样pointer[0]就可以接受左边前移的，从而那个原来的节点最后一个值就舍去不用
        // !!!!!!!!!!!!!!!!!!!!
        ...
        m_Keys[m_Count] = GetPointer(m_Count + 1) ->GetElement(1);
...
bool BPlusTree::Insert(KEY_TYPE data) {
    ...
    // 叶子节点没有满，新建叶子节点，并把原节点的后一半数据切割给新节点
    if (poldNode -> GetCount() < MAXNUM_DATA) {
        return poldNode ->Insert(data);
    }

    // 原叶子节点已经满了，新建叶子节点，并把原节点后一半数据切到新节点
    ...
    if (pFather == NULL) {
        ...
        pNode1 -> SetPointer(1, poldNode); // 指针1指向原节点
        pNode1 -> SetElement(1, key); // 设置键
        pNode1 ->SetPointer(2, pnewNode);
        poldNode ->SetFather(pNode1); //指定父节点
        pnewNode ->SetFather(pNode1); //指定父节点
        pNode1 ->SetCount(1);
    }
...
bool BPlusTree::Delete(KEY_TYPE data) {
    ...
    if (poldNode ->GetCount() >= ORDER_V) {
        for (int i = 1; (data >= pFather ->GetElement(i)) && (i <= pFather->GetCount()); i++) {
            if (pFather ->GetElement(i) == data) {
                pFather ->SetElement(i, poldNode ->GetElement(1));// 更改为叶子节点新的第一个元素
            }
        }
        return true;
    }
    ...
    if (pBrother ->GetCount() > ORDER_V) {
        if (flag == FLAG_LEFT) { // 兄弟在左边，移动最后一个数据
